...
def register(router):
    ...
    async def get_notifications(current_user: dict = Depends(get_current_user)):
        ...
        await db.notifications.delete_many(
            {
                "user_id": user_id,
                "notification_type": {"$ne": "user_message_sent"},
                "$or": [
                    {
                        "read": True,
                        "$or": [
        ...
        # Sent copies are always marked read; keep them for the unread window, not the 5-day read prune.
        await db.notifications.delete_many(
            {
                "user_id": user_id,
        ...
        agg = await db.notifications.aggregate(
            [
                {"$match": {"user_id": user_id, "notification_type": {"$ne": "user_message_sent"}}},
                {
                    "$facet": {
                        "notifications": [
                            {"$sort": {"created_at": -1}},
