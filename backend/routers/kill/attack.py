...
async def _build_active_attacks_list(
    ...
    *,
    ...
    - Never leak `traveling_to` (destination city). Hunters only see current city.
    """
    ...
    for attack in attacks:
        ...
        if attack["status"] == "found":
            ...
            in_transit = bool(_in_transit_destination(tu_found))
...
async def get_attack_status(
    ...
    if attack["status"] == "found" and attack.get("target_id"):
        ...
        in_transit = bool(_in_transit_destination(tu_status))
    else:
        in_transit = False
    chase_city = attack.get("location_state")
        ...
        if target.get("is_npc"):
            ...
            if hitlist_entry:
                ...
                await db.users.update_one(
                    {"id": victim_id},
                    {
                        "$set": {"is_dead": True, "dead_at": now_iso, "money": 0, "health": 0, "health_regen_last_at": now_iso, **player_death_set_fields()},
