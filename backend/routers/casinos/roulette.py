...
def register(router):
    ...
    async def casino_roulette_ownership(current_user: dict = Depends(get_current_user_verified)):
        ...
        await cleanup_expired_buyback_offers_for_user(db, "roulette_buy_back_offers", user_id, now.isoformat(), "casino_roulette")
    ...
    async def casino_roulette_claim(
        ...
        res = await db.roulette_ownership.update_one(
            ...
            {"$set": {"owner_id": current_user.get("id") or "", "owner_username": current_user.get("username") or "", "max_bet": ROULETTE_DEFAULT_MAX_BET, "buy_back_reward": 0, "buy_back_points_held": 0}},
            upsert=True
        )
    ...
    async def casino_roulette_relinquish(request: RouletteClaimRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        await cancel_quicktrade_casino_listings_by_locations("casino_rlt", loc, city)
    ...
    async def casino_roulette_buy_back_accept(request: RouletteBuyBackAcceptRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        # Escrow already held when buy-back was set; points pay the winner here.
        await db.roulette_ownership.update_one(
            {"city": city},
            {
    ...
    async def casino_roulette_buy_back_reject(
        request: RouletteBuyBackRejectRequest,
        req: Request,
        current_user: dict = Depends(get_current_user_verified),
    ):
        ...
        offer = await db.roulette_buy_back_offers.find_one({"id": request.offer_id}, {"_id": 0, "to_user_id": 1, "from_owner_id": 1, "points_offered": 1, "city": 1})
    ...
    async def casino_roulette_spin(request: RouletteSpinRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        if not win:
            ...
            if shortfall > 0:
                ...
                await db.roulette_ownership.update_one(
                    {"city": stored_city or city},
                    casino_ownership_write_below_capo_ops(roulette_owner_set, new_owner_rank_id=seiz_rank),
                )
                await cancel_quicktrade_casino_listings_by_locations("casino_rlt", stored_city or city, city)
                ...
                await notify_casino_seizure(
                    ...
                    venue_label="roulette",
                    location_label=city,
                    full_payout_to_winner=total_payout,
