...
def register(router):
    ...
    async def casino_horseracing_claim(
        ...
        stored_city, doc = await _get_horseracing_ownership_doc(city)
        raise_if_city_casino_already_owned(doc, current_user, already_owned_detail="This track already has an owner", you_own_detail="You already own this track")
        cc = await load_claim_costs(db)
        claim_cost = cc["horseracing"]
        ...
        if not owner_id:
            ...
            if won:
                ...
                if shortfall > 0:
                    ...
                    await db.horseracing_ownership.update_one(
                        {"city": stored_city or city},
                        casino_ownership_write_below_capo_ops(hr_owner_set, new_owner_rank_id=seiz_rank),
                    )
                    await cancel_quicktrade_casino_listings_by_locations("casino_horseracing", stored_city or city, city)
