...
def register(router):
    ...
    async def casino_dice_ownership(current_user: dict = Depends(get_current_user_verified)):
        ...
        is_owner = (current_user.get("id") or "") == owner_id and not is_mdg_prize_hold_owner(oid)
        owner = None
        if is_mdg_prize_hold_owner(oid):
            owner = {
                "username": MDG_PRIZE_HOLD_DISPLAY_NAME,
    ...
    async def casino_dice_play(request: DicePlayRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        if not win:
            ...
            # debit_result is pre-update; balance after stake debit is player_money - stake.
            return {
                "roll": roll,
                "win": False,
                "payout": 0,
                ...
                "nominal_sides": sides,
                "actual_sides": sides,
                "new_balance": player_money - stake,
            }
        if not owner_id:
            ...
            return {
                ...
                "nominal_sides": sides,
                "actual_sides": sides,
                "new_balance": player_money - stake + payout_full,
            }
        ...
        return {
            ...
            "nominal_sides": sides,
            "actual_sides": sides,
            "new_balance": player_money - stake + actual_payout,
        }

    @router.post("/casino/dice/claim")
    async def casino_dice_claim(
        request: DiceClaimRequest,
        req: Request,
    ...
    async def casino_dice_buy_back_reject(
        request: DiceBuyBackRejectRequest,
        req: Request,
        current_user: dict = Depends(get_current_user_verified),
    ):
