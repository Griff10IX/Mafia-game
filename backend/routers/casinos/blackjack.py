...
async def _blackjack_auto_finish_game(game: dict, current_user: dict):
    ...
    if dealer_total > 21:
        ...
        if head_family_id and edge_lose > 0:
            ...
            if el_tr > 0:
                await db.families.update_one({"id": head_family_id}, {"$inc": {"treasury": el_tr, "state_head_income.blackjack": el_tr}})
        if owner_id:
...
def register(router):
    ...
    async def casino_blackjack_ownership(current_user: dict = Depends(get_current_user_verified)):
        ...
        await cleanup_expired_buyback_offers_for_user(db, "blackjack_buy_back_offers", user_id, now.isoformat(), "casino_blackjack")
    ...
    async def casino_blackjack_claim(
    ...
    async def casino_blackjack_relinquish(request: RouletteClaimRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        await cancel_quicktrade_casino_listings_by_locations("casino_blackjack", loc, city)
    ...
    async def casino_blackjack_buy_back_accept(request: BlackjackBuyBackAcceptRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        await db.blackjack_ownership.update_one(
            {"city": city},
            {
                "$set": {"owner_id": from_owner_id, "owner_username": from_username, "max_bet": 0, "buy_back_reward": 0, "buy_back_points_held": 0},
                "$unset": {"below_capo_acquired_at": ""},
            },
        )
        cnorm = _normalize_city_for_blackjack(str(city or "").strip()) if city else ""
        await cancel_quicktrade_casino_listings_by_locations("casino_blackjack", city, cnorm or None)
        _invalidate_ownership_cache(current_user.get("id") or "")
    ...
    async def casino_blackjack_buy_back_reject(
        request: BlackjackBuyBackRejectRequest,
        req: Request,
        current_user: dict = Depends(get_current_user_verified),
    ):
        require_protection_revoke_confirm(current_user, reason="casino_buyback_reject", request=req)
        offer = await db.blackjack_buy_back_offers.find_one({"id": request.offer_id}, {"_id": 0, "to_user_id": 1, "from_owner_id": 1, "points_offered": 1, "city": 1})
    ...
    async def casino_blackjack_send_to_user(request: RouletteSendToUserRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        await send_notification(
            target.get("id") or "",
            "Casino transferred",
            f"{sender_name} sent you the blackjack table in {loc}.",
            "reward",
        )
    ...
    async def casino_blackjack_sell_on_trade(request: DiceSellOnTradeRequest, current_user: dict = Depends(get_current_user_verified)):
        ...
        await ensure_no_duplicate_casino_quicktrade_listing(
            "casino_blackjack", city, current_user.get("id") or ""
        )
        listing_id = ObjectId()
        casino_property = {
            "_id": listing_id,
        ...
        if payout > 0:
            if owner_id and result in ("win", "dealer_bust"):
                ...
                if shortfall > 0:
                    ...
                    await db.blackjack_ownership.update_one(
                        {"city": stored_city_bj or bj_city},
                        casino_ownership_write_below_capo_ops(bj_owner_set2, new_owner_rank_id=seiz_rank),
                    )
                    bj_norm = _normalize_city_for_blackjack(str(bj_city or "").strip()) if bj_city else ""
                    await cancel_quicktrade_casino_listings_by_locations("casino_blackjack", stored_city_bj or bj_city, bj_norm or None)
                    # Track casino seizure stats
                    await db.users.update_one({"id": current_user.get("id") or ""}, {"$inc": {"casinos_seized": 1}})
                    await db.users.update_one({"id": owner_id}, {"$inc": {"casinos_lost": 1}})
                    await notify_casino_seizure(
