...
def register(router):
    ...
    async def snake_score(payload: SnakeScoreRequest, current_user: dict = Depends(get_current_user)):
        ...
        if not skip_session:
            ...
            pl = await get_plays_left(db, user_id=uid, game=SNAKE_GAME)
            if pl["plays_left"] == 0:
                raise HTTPException(
                    status_code=429,
        ...
        try:
            await log_activity(
                ...
                "minigame_snake",
                {"score": score, **rewards_applied},
            )
        except Exception:
            pass

        try:
            await log_minigame_payout(current_user["id"], current_user.get("username", "?"), "snake", score, rewards_applied)
        except Exception:
            pass
