...
def _verify_image_magic_bytes(data: bytes, declared_mime: Optional[str]) -> tuple[bool, str]:
    """Ensure raw bytes are JPEG, PNG, GIF, or WebP (not HTML/scripts). Optional declared_mime must match."""
    if len(data) < 12:
        return False, "Invalid or empty image file"

    def sniff() -> Optional[str]:
...
def _validate_car_image_data_url(url: str) -> tuple[bool, str]:
    """Validate data:image/...;base64,... custom car image."""
    ...
    return _verify_image_magic_bytes(decoded, mime_type)


def _car_image_url_host_is_public(url: str) -> tuple[bool, str]:
    """Reject SSRF: only public DNS-resolved IPs (no loopback, RFC1918, link-local, metadata)."""
    parsed = urlparse(url)
...
async def _validate_car_image_http_url(url: str) -> tuple[bool, str]:
    """
    Remote URLs: HTTPS/HTTP only, public IP, stream a prefix and verify binary image signatures.
    Blocks HTML pages, executables, and internal network targets (SSRF).
    """
    import httpx
    ...
    ok_host, host_err = _car_image_url_host_is_public(url_stripped)
    if not ok_host:
    ...
    try:
        async with httpx.AsyncClient(
            ...
            async with client.stream("GET", url_stripped, headers=headers) as r:
                ...
                ok_final, err_final = _car_image_url_host_is_public(str(r.url))
                if not ok_final:
    ...
    ok_magic, magic_err = _verify_image_magic_bytes(data, declared)
    if not ok_magic:
...
async def validate_custom_car_image_value(url: str) -> tuple[bool, str]:
    """
    Full validation for stored custom car image (data URL or https image link).
    """
    ...
    if url.lower().startswith("data:"):
        return _validate_car_image_data_url(url)

    return await _validate_car_image_http_url(url)
