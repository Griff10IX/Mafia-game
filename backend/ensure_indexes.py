...
async def ensure_all_indexes(db):
    ...
    try:
        ...
        await db.sports_betting_ownership.create_index("owner_id", unique=True, sparse=True)
        ...
        await db.store_cash_purchase_logs.create_index([("username", 1), ("created_at", -1)])
        await db.store_cash_purchase_logs.create_index([("purchase_kind", 1), ("created_at", -1)])
        await db.store_cash_purchase_logs.create_index([("client_ip", 1), ("created_at", -1)])
        await db.store_cash_purchase_logs.create_index([("email", 1), ("created_at", -1)])
        ...
        await db.notifications.create_index([("user_id", 1), ("notification_type", 1), ("created_at", -1)])
        ...
        from utils.last_man_standing import ensure_lms_indexes
        await ensure_lms_indexes(db)
