...
async def require_turnstile_for_ent_join(
    db,
    *,
    request: Request,
    current_user: dict,
    ...
    """
    When main.ent_join_turnstile_enabled is True and Turnstile keys are configured,
    require a valid token on entertainer game joins (anti-bot layer 2, admin-switchable).
    ...
    secret = turnstile_secret()
    if not site_key or not secret:
        logger.warning("ent_join_turnstile enabled but site key or TURNSTILE_SECRET_KEY missing")
        await log_captcha_turnstile_failure(
            db,
            request=request,
            current_user=current_user,
            reason="misconfigured",
            detail="ent_join_turnstile enabled but site key or TURNSTILE_SECRET_KEY missing",
        )
        raise HTTPException(
            status_code=503,
            detail="Captcha is enabled but the server is not fully configured (TURNSTILE_SECRET_KEY and site key).",
    ...
    if not raw:
        ...
        raise HTTPException(
            status_code=400,
            detail="Complete the captcha before joining.",
        )
