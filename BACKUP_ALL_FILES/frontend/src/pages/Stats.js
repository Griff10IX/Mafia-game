  ...
  return (
    <div className="bg-card border border-border rounded-sm overflow-hidden">
      <div className="px-4 py-2 bg-secondary/40 flex items-center justify-between">
        <div className="text-xs uppercase tracking-widest text-mutedForeground">{title}</div>
      </div>
      <div className="divide-y divide-border">
        {safeRows.map((r) => (
          <div key={r.label} className="grid grid-cols-12 px-4 py-2 text-xs">
            <div className="col-span-7 text-mutedForeground">{r.label}</div>
            <div className="col-span-5 text-right font-mono text-foreground">{r.value}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6" data-testid="stats-page">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs uppercase tracking-[0.3em] text-mutedForeground">Overview</div>
          <h1 className="text-3xl md:text-4xl font-heading font-bold text-primary mt-2 flex items-center gap-2">
            <TrendingUp size={22} className="text-primary" />
            Stats
          </h1>
          <p className="text-sm text-mutedForeground mt-1">Economy, users, ranks, and recent kills.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-card border border-border rounded-sm overflow-hidden">
          <div className="px-4 py-2 bg-secondary/40 flex items-center justify-between">
            <div className="text-xs uppercase tracking-widest text-mutedForeground">Rank Stats</div>
          </div>
          <div className="divide-y divide-border">
            {rankStats.map((r) => (
              <div key={r.rank_id} className="grid grid-cols-12 px-4 py-2 text-xs">
                <div className="col-span-6 text-foreground font-semibold truncate">{r.rank_name}</div>
                <div className="col-span-3 text-center text-emerald-400 font-mono">{formatNumber(r.alive)}</div>
                <div className="col-span-3 text-right text-mutedForeground font-mono">{formatNumber(r.dead)}</div>
              </div>
            ))}
            {rankStats.length === 0 && (
              <div className="px-4 py-4 text-sm text-mutedForeground">No rank data yet.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-sm overflow-hidden">
        <div className="px-4 py-2 bg-secondary/40 flex items-center justify-between">
          <div className="text-xs uppercase tracking-widest text-mutedForeground">Last 15 Kills</div>
        ...
        <div className="border-t border-border">
          <div className="grid grid-cols-12 bg-secondary/20 text-xs uppercase tracking-wider text-mutedForeground px-4 py-2">
          ...
          {recentKills.length === 0 ? (
            <div className="px-4 py-4 text-sm text-mutedForeground">No kills yet.</div>
          ) : (
            recentKills.map((k) => (
              <div key={k.id} className="grid grid-cols-12 px-4 py-2 border-t border-border text-xs">
                <div className="col-span-4 text-foreground font-semibold truncate">{k.victim_username}</div>
