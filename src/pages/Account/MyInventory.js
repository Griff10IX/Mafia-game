...
const formatChipValue = (format, v) => {
  if (format === 'money') return fmtMoney(v);
  if (format === 'rp') return `+${fmtNum(v)} RP`;
  if (format === 'points') return `${fmtNum(v)} pts`;
  if (format === 'dur') return fmtDuration(v);
  return fmtNum(v);
};
const CHIP_VALUE_CLASS = {
