    ...
    <div className={`space-y-6 ${styles.pageContent}`} data-testid="dead-alive-page">
      ...
      <div className="relative da-fade-in">
        <p className="text-[10px] text-zinc-500 font-heading italic max-w-xl">
          Receive 95% of a dead account’s money and points here (5% tax). One-time per dead account.
        </p>
      </div>
      ...
      <div className={`relative ${styles.panel} rounded-lg overflow-hidden max-w-md shadow-lg shadow-primary/5 border border-primary/20`}>
        ...
        <div className="px-4 py-2.5 bg-amber-500/15 border-b border-amber-500/30 flex items-center gap-2">
          ...
          <span className="text-[10px] font-heading font-bold text-amber-200/90">You must be logged into your new account. Enter the dead account’s username and password. You receive 95% of its money and points (5% tax).</span>
        </div>
        <form onSubmit={handleRetrieve} className="p-6 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-primary/20 text-primary rounded-sm font-heading font-bold uppercase tracking-wider py-3 border border-primary/40 hover:bg-primary/30 transition-smooth disabled:opacity-50"
            data-testid="retrieve-submit"
          >
            <KeyRound size={18} />
            {loading ? 'Transferring...' : 'Transfer 95% (money & points, 5% tax)'}
          </button>
        </form>
      ...
      <div className={`relative ${styles.panel} rounded-lg overflow-hidden max-w-2xl border border-primary/20`}>
        ...
        <div className="p-6">
          <ul className="space-y-2 text-sm text-mutedForeground font-heading">
            ...
            <li className="flex items-start gap-2"><span className="text-primary mt-0.5">◆</span> Here enter your <strong className="text-foreground">dead account</strong> username and password. This account receives <strong className="text-foreground">95% of that account’s money and points</strong> (5% tax; as it had when it died).</li>
