    ...
    <div className={`space-y-4 ${styles.pageContent}`} data-testid="slots-page">
      ...
      <div className={`${styles.panel} rounded-md overflow-hidden border border-primary/20`}>
        ...
        <div className="p-3">
          ...
          <p className="text-[10px] text-mutedForeground mt-2">0.05% house edge on wins. One machine per state.</p>
                ...
                <p className="text-[10px] text-mutedForeground font-heading">
                  {(ownership?.next_draw_at || config.next_draw_at)
                    ? (() => {
                        ...
                        const intervalLabel = config.draw_interval_minutes === 1 ? 'every 1 min' : `every ${config.ownership_hours}h on the hour`;
                        const suffix = minsUntil != null && minsUntil < 120 ? ' (runs automatically when due)' : ` (${intervalLabel})`;
