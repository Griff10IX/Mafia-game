    ...
    <div className={`space-y-6 ${styles.pageContent}`} data-testid="organised-crime-page">
      ...
      {status?.pending_heist && (
        <div className="rounded-md overflow-hidden max-w-2xl mx-auto border border-primary/30" style={{ backgroundColor: 'var(--gm-bg-top)' }}>
          <div className="px-4 py-2 border-b flex items-center justify-between" style={{ borderColor: 'var(--gm-border)', color: 'var(--gm-gold)' }}>
            ...
            <UserCheck size={18} style={{ color: 'var(--gm-gold)' }} />
          </div>
          <div className="p-4 space-y-3">
            {ROLE_IDS.map((roleId) => {
              ...
              return (
                <div key={roleId} className="flex flex-wrap items-center gap-2">
                  <span className="w-24 text-xs font-heading font-bold capitalize shrink-0" style={{ color: 'var(--gm-gold)' }}>{roleId}</span>
                  <span className="text-xs font-heading" style={{ color: 'var(--gm-text)' }}>{displayVal}</span>
                  {inv && <span className="text-xs font-heading" style={{ color: statusStr === 'accepted' ? 'var(--gm-profit)' : 'var(--gm-muted)' }}>({statusStr})</span>}
                  {canClear && (
                    <button type="button" onClick={() => cancelInvite(inv.invite_id)} className="flex items-center gap-1 text-xs font-heading text-amber-500 hover:text-amber-400" title="Clear slot">
                  ...
                  {isEmpty && (
                    <>
                      {!editing ? (
                        <button type="button" onClick={() => setPendingSlotEdit({ role: roleId, value: '' })} className="text-xs font-heading text-primary hover:underline">Set NPC or invite</button>
                      ) : (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            style={{ backgroundColor: 'var(--gm-card)', border: '1px solid var(--gm-border)', color: 'var(--gm-gold)' }}
                          />
            ...
            <div className="pt-2">
              <button type="button" onClick={runFromPending} disabled={executing || onCooldown || (status.pending_invites || []).some((i) => i.status === 'pending') || ROLE_IDS.some((r) => status.pending_heist[r] == null || status.pending_heist[r] === '')} className="bg-gradient-to-b from-primary to-yellow-700 text-primaryForeground hover:opacity-90 rounded-sm font-heading font-bold uppercase tracking-widest px-4 py-2 text-sm border border-yellow-600/50 disabled:opacity-50">
            ...
            <div key={roleId} className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {['self', 'npc', 'invite'].map((opt) => (
                  <label key={opt} className="flex items-center gap-1.5 cursor-pointer" style={{ color: 'var(--gm-gold)' }}>
                    <input
                      type="radio"
                      name={`slot-${roleId}`}
                      ...
                      style={{ accentColor: 'var(--gm-gold)', borderColor: 'var(--gm-gold)', outlineColor: 'var(--gm-gold)' }}
                    />
      ...
      <div className="rounded-md overflow-hidden max-w-2xl mx-auto border border-primary/30" style={{ backgroundColor: 'var(--gm-bg-top)' }}>
        <div className="px-4 py-2 border-b flex items-center justify-between" style={{ borderColor: 'var(--gm-border)', color: 'var(--gm-gold)' }}>
          ...
          <span className="text-xs font-heading font-bold" style={{ color: pctTotal === 100 ? 'var(--gm-gold)' : 'var(--gm-muted)' }}>Total: {pctTotal}%</span>
        </div>
        <div className="p-4 flex flex-wrap items-center gap-4">
          {ROLE_IDS.map((roleId) => (
            <div key={roleId} className="flex items-center gap-2">
              <span className="text-xs font-heading capitalize w-20" style={{ color: 'var(--gm-gold)' }}>{roleId}</span>
              <input
                type="number"
                min={0}
                max={100}
                ...
                style={{ backgroundColor: 'var(--gm-card)', border: '1px solid var(--gm-border)', color: 'var(--gm-gold)' }}
              />
              <span className="text-xs font-heading" style={{ color: 'var(--gm-gold)' }}>%</span>
            </div>
          ))}
        </div>
        {pctTotal !== 100 && (
