  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { path: '/game/system-ai', label: 'System AI' },
        { path: '/game/guide', label: 'Game Guide' },
...
function SameRouteAwareLink({ to, onClick, onMouseEnter, onFocus, onPointerDown, onTouchStart, ...rest }) {
  ...
  const warmPointer = () => {
    ...
    if (warmCrimes && !getCrimesPrefetch() && !isClientJailed()) {
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    const bump = () => setPortalNavTick((t) => t + 1);
  ...
  useEffect(() => {
    if (!staffPortalEnabled || !staffLoginSession || (!isAdmin && !isModerator)) return undefined;
  ...
  const navItems = [
    ...
    { path: '/game/system-ai', icon: Bot, label: 'System AI' },
    { path: '/game/guide', icon: BookOpen, label: 'Game Guide' },
    ...
    <div className="space-y-0.5">
      ...
      {rankingOpen && (
        <div className={`space-y-0 ${styles.sidebarSubmenuBorder}`}>
          <SameRouteAwareLink to="/crime/crimes" onClick={() => setSidebarOpen(false)} onMouseEnter={() => { if (!isClientJailed()) api.get('/crimes').then((r) => setCrimesPrefetch(r.data)).catch(() => {}); }} onFocus={() => { if (!isClientJailed()) api.get('/crimes').then((r) => setCrimesPrefetch(r.data)).catch(() => {}); }}
          ...
          {pocketMenuOpen && (() => {
            ...
            const menuItems = {
              you: [
                ...
                { path: '/game/system-ai', label: 'System AI', Icon: Bot },
                { path: '/game/guide', label: 'Game Guide', Icon: BookOpen },
                { path: '/account/autorank', label: 'Auto Rank', Icon: Sparkles },
              ...
              <div data-layout="bottom-nav-submenu" className="absolute bottom-full left-0 right-0 z-[52] border-t border-primary/20 shadow-2xl max-h-[60vh] overflow-y-auto"
                ...
                <div className="p-2 grid grid-cols-3 gap-1.5">
                  {group.items.map((sub, idx) => {
                    ...
                    const prefetchCrimes = sub.path === '/crime/crimes' ? () => { if (!isClientJailed()) api.get('/crimes').then((r) => setCrimesPrefetch(r.data)).catch(() => {}); } : undefined;
